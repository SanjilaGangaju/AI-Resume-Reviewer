...
const Dashboard = ({ currentUser, dbloading, setdbloading }) => {
  ...
  return (
    <div className='flex bg-indigo-100 items-center flex-col justify-center p-2 h-full'>
      {dbloading ? (
        <div className='h-94 flex items-center justify-center'>
          <CircleLoader color="blue" size={100} />
        </div>
      ) : feedbacks.length > 0 ? (
        <div className='flex flex-col gap-6 items-center justify-center p-1'>
          <h2 className='text-center text-2xl text-indigo-400'>Feedback Dashboard</h2>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-5 '>
            {feedbacks.map(f => (
              <div key={f.id} className='rounded ' >
                <p className='text-center text-[0.65rem] bg-gray-50  text-blue-700 mb-3  py-2'>Feedback of Resume: {f.resumeFileName}</p>
                <div>
                  <div className='px-2 py-4 flex flex-col bg-gray-50 gap-4'>
                    ...
                    <div className="flex flex-col gap-5">
                      <div className='flex gap-4 justify-start items-center  px-2 '>
                        <div >
                          <div className='relative bg-yellow-100 h-20 w-[4rem] rounded-b-full '>
                            <div style={{ height: `${f.feedbackText.totalScore}%` }} className='absolute flex items-center justify-center font-bold bg-indigo-300 w-full bottom-0 rounded-b-full text-gray-50 text-[0.6rem]'>
                              {f.feedbackText.totalScore}/100
                            </div>
                          </div>
                        </div>
                        <div>
                          <p className='text-s font-semibold text-gray-500'>Your Resume Score</p>
                          <p className='text-[0.6rem] text-gray-500'>This score is calculated based on the categories listed below</p>
                        </div>
                      </div>

                      <div className=' rounded-xl sm:w-full flex flex-col gap-1 '>
                        {Object.entries(f.feedbackText.scores).map(([key, category]) => {
                          const { label, color, bg } = scoreLabel(category.score)
                          return (
                            <div key={key}>
                              <div className='flex  flex-row gap-2 bg-indigo-50  px-4 py-3 rounded-full items-center justify-between'>
                                ...
                                <span className={`text-[0.6em] text-gray-500 ${color}`}>{category.score} <span className='text-black'>/ 100</span></span>
                              </div>
                            </div>
                          )
                        })}
                    ...
                    <div className='bg-indigo-50 flex flex-col gap-2 rounded-xl px-5 py-3'>
                      ...
                      {atsaccordin.includes(f.id) && (<>
                        ...
                        <p className='text-[0.6rem] text-gray-400 italic'>Keep refining your resume to improve your chances of getting past ATS filters and into the hands of recruiters</p>
                      </>)}
                    </div>
                      ...
                      {Object.entries(f.feedbackText.scores).map(([key, category]) => (
                        <div key={key}>
                          <div>
                            ...
                            {accordin.includes(key) && (
                              <div className='flex flex-col gap-2 px-5 justiy-center bg-indigo-50 rounded py-5'>
        ...
        <div className='flex flex-col items-center gap-7 m-12'>
          <p>🥺 No feedback found yet. Upload your resume to get started!</p>
          <button className='bg-indigo-300 text-white p-1 rounded hover:bg-indigo-500'>
